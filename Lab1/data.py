...
def sample_gmm_2d(ncomponents, nclasses, nsamples):   #-> iz svake od ncomponents Gaussovih razdioba generira (uzrokuje) nsamples podataka
  # create the distributions and groundtruth labels
  Gs=[]
  Ys=[]
  ...
  # sample the dataset -> iz svake distribucije (iz Gs za X  i  Ys za Y_) uzorkovati traženi broj podataka
  X = np.vstack([G.get_sample(nsamples) for G in Gs])
  Y_= np.hstack([[Y]*nsamples for Y in Ys])
