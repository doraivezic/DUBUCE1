...
class FC(Layer):
  ...
  def backward_params(self, grads):
    ...
    """
    # TODO - done
    grad_weights = grads.T @ self.prev_inputs
...
class ReLU(Layer):
  ...
  def backward_inputs(self, grads):
    ...
    """
    # TODO - done
    return np.where( self.prev_inputs<0., 0., grads)
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    e = np.exp(x)
    e /= np.sum(e, axis=1)[:,None]   #suma svih elemenata (po klasama) na exp
  ...
  def backward_inputs(self, x, y):
    ...
    e = np.exp(x)
    e /= np.sum(e, axis=1)[:,None]   #softmax(x)
